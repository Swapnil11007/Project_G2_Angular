...
export class SnowsitesComponent implements OnInit {
  ...
  Addtrips(product: any){
    this.cart={prodName:product.snowName , emailID:this.userservice.getEmail() , imgPath:product.snowPath ,travelDate:this.doj, quantity:this.QTY, totalAmount:(this.QTY*product.snowPrice)};
  ...
  Addtrips1(product: any){
    this.cart={prodName:product.snowName , emailID:this.userservice.getEmail() , imgPath:product.snowPath ,travelDate:this.doj1, quantity:this.QTY, totalAmount:(this.QTY*product.snowPrice1)};
    this.service.setDashboard(this.cart).subscribe((data:any)=>{console.log('trip added');});
    alert(this.cart.prodName+" Added to Cart");
